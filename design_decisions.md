...
### 3.3 Reranking
Decision:
- Add rerank signals and support a learned reranker stage (CrossEncoder) for final top-N ordering.

Why:
- Metadata-based rerank helps operational relevance.
- With the new dataset containing richer textual fields (`Incident Details`, `Description`, `Solution`),
  cross-encoder reranking can materially improve top-N precision.

Trade-off:
- Slightly more scoring complexity, latency, and tuning effort.
...
What A2A context means:
- A2A (Agent-to-Agent) context is the structured handoff payload passed between L1, L2, and RCA agents.
- It contains the essential working state for escalation, such as:
  - user query summary
  - filters used
...
Decision:
- Use PostgreSQL for users, sessions, messages, usage, feedback, incident metadata, and durable A2A context.
...
### 9.3 A2A Traceability
Decision:
- Persist A2A handoff context to support replayability and auditability of escalations.

Why:
- Makes agent escalation behavior explainable for demos, debugging, and production incident reviews.
- Enables future analytics on handoff quality and bottlenecks between L1/L2/RCA tiers.
...
Practical note for the current dataset:
- `incident_response_dataset_150_rows.xlsx - Incident Data.csv` includes semantically rich text columns:
  `Incident Details`, `Description`, and `Solution`.
- This makes CrossEncoder reranking a strong near-term enhancement (not just long-term),
  especially for disambiguating similar incidents with different remediation steps.
- Recommended near-term rollout:
  1. Retrieve top-50 candidates via BM25 + Milvus.
  2. Rerank top-50 with `cross-encoder/ms-marco-MiniLM-L6-v2`.
  3. Return top-5 after rerank with grounded resolution snippets.
